...
| 图层       | 皮肤不透明度、器官、骨骼、血管、神经、肌肉开关；可选大图层按需加载              |
...
这是一套**可运行的医学科普演示系统**，不是经过临床验证的医学教学产品、诊断工具或患者模型。真实器官/骨骼网格来自 Z-Anatomy；皮肤来自 BodyParts3D，两者进行了近似配准。器官网格保留原始解剖细节，颜色、可见性、分类和标签作了科普化处理，尚需解剖专业人员逐结构校核才能宣称达到医学级精度。
...
首屏五个模型约 **8.9 MB / 525 个网格**；肌肉、血管和神经模型在启用时单独加载。模型与 Draco 解码器本地托管，不依赖模型 CDN。界面字体可使用在线 Google Fonts，离线时回退到本机字体。

医学内容附有 **41 个 NHS / NIH 等权威健康资料来源**。规则提醒用于介绍危险信号，**未触发规则不意味着安全**；知识库覆盖范围有限，不能给出明确诊断、处方、剂量或治疗保证。

模型主体采用 CC BY-SA 许可；上游也列有耳部、肾脏等组件的非商业许可声明，因此不能把整套资产视为已经获得商业授权。详见 [完整模型来源与许可](public/models/ATTRIBUTION.md)。正式商用前，应逐组件确认授权或替换受限模型，并进行医学内容审核。
