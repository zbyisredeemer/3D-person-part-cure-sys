...
| 疾病百科   | 64 个疾病主题；按器官与分类组合筛选、疾病/症状/中英文器官搜索、基础照护和注意事项 |
...
疾病目录在桌面端独立滚动，在手机端使用两列滚动列表，并提供“阅读详情 / 返回疾病目录”入口。从器官档案跳转时自动显示对应疾病；切到 3D 或其他栏目再返回，保留本次页面会话中的搜索、器官/分类筛选与选中疾病，刷新后恢复默认状态。疾病和药物搜索支持忽略首尾空白、按空白分隔的多个关键词组合匹配；结果计数和重置入口帮助恢复全部主题。器官收藏搜索会区分尚未收藏与没有匹配结果。
